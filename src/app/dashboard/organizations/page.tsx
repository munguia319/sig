...
async function OrganizationsPage() {
  ...
  if (organizations.length === 1) {
    ...
    const href = getAppHomeUrl(organization.slug);
...
function getAppHomeUrl(slug: string) {
  const protocol = configuration.production ? 'https' : 'http';
  const siteUrl = configuration.site.siteUrl;
  const appHomePath = configuration.paths.appHome;

  if (!siteUrl) {
    throw new Error('Missing siteUrl in configuration');
  }

  const { hostname, port } = new URL(siteUrl);
  const domain = port ? `${hostname}:${port}` : hostname;

  return `${protocol}://${slug}.${join(domain, appHomePath)}`;
}
