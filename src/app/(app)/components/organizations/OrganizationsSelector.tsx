...
import { setCurrentOrganizationAction } from '~/lib/organizations/actions';
import useCsrfToken from '~/core/hooks/use-csrf-token';

const OrganizationsSelector = () => {
  const [isOrganizationModalOpen, setIsOrganizationModalOpen] = useState(false);
  const [isSelectOpen, setIsSelectOpen] = useState(false);
  const [isChangingOrganization, startTransition] = useTransition();
  ...
  const onOrganizationChange = useCallback(async (organizationId: string) => {
    startTransition(() => {
      setCurrentOrganizationAction({ organizationId, csrfToken });
    });
  }, []);
