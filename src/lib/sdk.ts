...
class OrganizationSubscriptionSdk {
  ...
  /**
   * Checks if the subscription is for the given LS price ID.
   * @param variantId The LS variantId ID to check.
   */
  public async isPlan(variantId: number) {
  ...
  public async isActive() {
    ...
    return ['active', 'on_trial'].includes(this.status);
  }
