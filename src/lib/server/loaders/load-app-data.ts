...
import getOrganizationSlug from '~/lib/organizations/get-organization-slug';
...
const loadAppData = cache(async () => {
  try {
    ...
    if (!slug) {
      redirect(`${configuration.paths.appHome}/organizations`);
    }
    ...
    const [userRecord, organizationData] = await Promise.all([
      ...
      getCurrentOrganization({ slug, userId }),
    ]);
