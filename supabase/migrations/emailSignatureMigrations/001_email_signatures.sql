...
-- Allow all users (public + authenticated) during development
...
-- Optional: auto-delete expired links daily at 3am UTC (requires pg_cron)
-- SELECT cron.schedule('delete-expired-signature-links', '0 3 * * *',
--   $$ DELETE FROM public.public_signature_links WHERE expires_at < now(); $$);
