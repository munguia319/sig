 ...
 * DozerSidebar
 * =============
 * Sidebar for "The Dozer" template.
 * Fields: Name, Title, Cell Phone, Office Phone, Email,
 *         Website, Promo Text, Promo Link Text, Promo Link URL,
 *         Logo URL, Brand Color, Social Links (Instagram, X, Facebook),
...
function toDozerFields(f: ReturnType<typeof useSignature>['fields']) {
  return {
    fullName: f.fullName,
    jobTitle: f.jobTitle,
    ...
    promoLinkText: f.promoLinkText,
    promoLinkUrl: f.promoLinkUrl,
    logoUrl: f.logoUrl,
...
export function DozerSidebar() {
  ...
  function handleDownload() {
    const html = generateDozerHtml(toDozerFields(fields));
  ...
  function handleCopy() {
    navigator.clipboard.writeText(generateDozerHtml(toDozerFields(fields)))
    ...
    <aside className="sidebar-shell">
      <div className="sidebar-header">
        ...
        <p className="text-[#888888] text-[11px] font-mono">
          {activeEmployeeId ? `Editing: ${fields.fullName || 'Untitled'}` : 'New signature'} · The Dozer
        </p>
      </div>
      ...
      <div className="sidebar-scroll">
        ...
        <AccordionSection icon={<User size={14} />} title="General Info & Assets" defaultOpen={true}>
          ...
          <FieldRow label="Full Name" id="d-fullName" value={fields.fullName} onChange={(v) => setField('fullName', v)} placeholder="John Dozer" />
          <FieldRow label="Job Title" id="d-jobTitle" value={fields.jobTitle} onChange={(v) => setField('jobTitle', v)} placeholder="Chief Signature Officer" />

          <div className="sidebar-section">
            <p className="field-section-label">Contact</p>
            <FieldRow label="Cell Phone" id="d-cellPhone" value={fields.cellPhone} onChange={(v) => setField('cellPhone', v)} placeholder="(000) 000-0000" type="tel" />
            <FieldRow label="Office Phone" id="d-officePhone" value={fields.officePhone} onChange={(v) => setField('officePhone', v)} placeholder="(000) 000-0000 ext 000" type="tel" />
            <FieldRow label="Email Address" id="d-email" value={fields.email} onChange={(v) => setField('email', v)} placeholder="name@company.com" type="email" />
            <FieldRow label="Website" id="d-website" value={fields.website} onChange={(v) => setField('website', v)} placeholder="www.company.com" />
          </div>

          <div className="sidebar-section">
            <p className="field-section-label">Promo Text</p>
            <p className="text-[11px] text-[#888888] mb-3 leading-relaxed">Italic text below the website. Renders as: "[Promo Text] [Link Text] with the ability to add a hyper link"</p>
            <FieldRow label="Promo Text" id="d-promoText" value={fields.promoText} onChange={(v) => setField('promoText', v)} placeholder="Custom text or promo copy here with a" />
            <FieldRow label="Link Text" id="d-promoLinkText" value={fields.promoLinkText} onChange={(v) => setField('promoLinkText', v)} placeholder="link" />
            <FieldRow label="Link URL" id="d-promoLinkUrl" value={fields.promoLinkUrl} onChange={(v) => setField('promoLinkUrl', v)} placeholder="https://company.com" />
          </div>

          <div className="sidebar-section">
            <p className="field-section-label">Assets</p>
            <FieldRow label="Logo URL" id="d-logoUrl" value={fields.logoUrl} onChange={(v) => setField('logoUrl', v)} placeholder="https://... (180×70 px PNG)" />
          </div>

          <div className="sidebar-section">
            <p className="field-section-label">Style</p>
            <label htmlFor="d-brandColor" className="field-label block mb-1">Brand Color</label>
            <p className="text-[11px] text-[#888888] mb-2 leading-relaxed">Controls email link color and promo hyperlink color.</p>
            <div className="flex items-center gap-2">
              <input id="d-brandColor" type="color" value={fields.brandColor} onChange={(e) => setField('brandColor', e.target.value)} className="w-8 h-8 rounded border border-[#e5e7eb] bg-white cursor-pointer p-0.5" />
        ...
        <AccordionSection icon={<Share2 size={14} />} title="Social Links" defaultOpen={true}>
          ...
          <FieldRow label="LinkedIn URL" id="d-linkedin" value={fields.linkedinUrl} onChange={(v) => setField('linkedinUrl', v)} placeholder="https://linkedin.com/in/..." />
          <FieldRow label="Instagram URL" id="d-instagram" value={fields.instagramUrl} onChange={(v) => setField('instagramUrl', v)} placeholder="https://instagram.com/..." />
          <FieldRow label="X / Twitter URL" id="d-twitter" value={fields.twitterUrl} onChange={(v) => setField('twitterUrl', v)} placeholder="https://x.com/..." />
          <FieldRow label="Facebook URL" id="d-facebook" value={fields.facebookUrl} onChange={(v) => setField('facebookUrl', v)} placeholder="https://facebook.com/..." />
        </AccordionSection>
        ...
        <AccordionSection icon={<Lock size={14} />} title="Confidentiality Clause" badge={fields.confidentialityEnabled ? 'ON' : undefined}>
          ...
          {fields.confidentialityEnabled && (
            <div className="mt-3">
              <label htmlFor="d-clause" className="field-label block mb-1">Clause Text</label>
              <textarea id="d-clause" value={fields.confidentialityText} onChange={(e) => setField('confidentialityText', e.target.value)} rows={6}
              ...
              <p className="text-[10px] text-[#aaaaaa] font-mono mt-1">Renders as small italic text below the social/logo row.</p>
            </div>
          )}
        ...
        <AccordionSection icon={<Link2 size={14} />} title="Tracking Parameters" badge={fields.trackingEnabled && activeUtm > 0 ? `${activeUtm} param${activeUtm > 1 ? 's' : ''}` : undefined}>
          ...
          {fields.trackingEnabled && (
            <div className="mt-3">
              <FieldRow label="utm_source" id="d-utmSource" value={fields.utmSource} onChange={(v) => setField('utmSource', v)} placeholder="email" />
              <FieldRow label="utm_medium" id="d-utmMedium" value={fields.utmMedium} onChange={(v) => setField('utmMedium', v)} placeholder="signature" />
              <FieldRow label="utm_campaign" id="d-utmCampaign" value={fields.utmCampaign} onChange={(v) => setField('utmCampaign', v)} placeholder="q1-outreach" />
              <FieldRow label="utm_content" id="d-utmContent" value={fields.utmContent} onChange={(v) => setField('utmContent', v)} placeholder="cta-button" />
            </div>
          )}
